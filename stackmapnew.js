...
window.StackMap = window.StackMap || {
    ...
    addPopups: function(){
        ...
        //    console.log('c', callno, location, collection)
         ...
         //   console.log('hold', holdingString)
            ...
            var headerLength = cells.length;
		$targetNode.data('headerlength', headerLength)
...
//                        jQuery('<li ><p class="SMemph">' + range + '</p></li><br>'),
                    ...
                    // add "map it" button
		    var headerlength = entries[j].data('headerlength')       
		    var parentEntry = entries[j].parent().parent().find('tr').eq(0)
	//	console.log(parentEntry, parentEntry.find('th').length)
		if (parentEntry.find('th').length === headerlength) {
			parentEntry.append('<th></th>')
		}
 
                    entries[j].append('<td style="width: 58px"><button class="SMbutton SMsearchbtn" type="button" onclick="StackMap.showPopup(\'SM' + StackMap.popupCounter + '\');" ><i class="fa fa-map-marker" aria-hidden="true"></i> Map it</button></td>');
